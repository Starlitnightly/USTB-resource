...
int test2() {
    ...
    if (p1 == 0) {          //child process
        ...
        if(px == 0){
          ...
          for(int i = 0; i < 500; i++){
            dprintf(pipeline[1], "LowUser 1 400\n");
          }

          for(int i = 0; i < 2000; i++){
            dprintf(pipeline[1], "HighUser 2 900\n");
          }

          for(int i = 0; i <  200000 - 500 - 2000; i++){
            dprintf(pipeline[1], "LowUser 1 300\n");
          }
          close(pipeline[1]);
          close(pipeline[0]);
          ...
          close(pipeline[0]);
          close(pipeline[1]);
          execlp("./score_board", "./score_board", NULL);
          perror("Execlp Failed: ");    //this line is not supposed to be executed
          exit(1);
        }
    } else if (p1 > 0) {       //parent process
